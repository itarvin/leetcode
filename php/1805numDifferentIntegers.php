...
class Solution {
    ...
    function numDifferentIntegers($word) {
        $result = array_unique(preg_split('/[a-z]+/s', $word));
        $new = [];
        foreach ($result as $key => $item) if (is_numeric($item) && !in_array($item, $new)) $new[] = preg_replace('/^0*/', '', $item);
        // var_dump($new);die;
...
$word = "0i00e";
$res = $object->numDifferentIntegers($word);
$t2 = microtime(true);
/****************************... 执行代码结束 ...****************************/
var_dump($res);
echo '<hr />';
