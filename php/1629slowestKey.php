...
class Solution {
    ...
    function slowestKey($releaseTimes, $keysPressed) {
        array_unshift($releaseTimes, 0);
        $result = [];
        for ($i=0; $i < strlen($keysPressed); $i++) $result[$keysPressed[$i].$i] = $releaseTimes[$i + 1] - $releaseTimes[$i];
        krsort($result);
...
$res = $object->slowestKey($releaseTimes, $keysPressed);
$t2 = microtime(true);
/****************************... 执行代码结束 ...****************************/
var_dump($res);
echo '<hr />';
